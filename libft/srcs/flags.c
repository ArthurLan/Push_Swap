...
/*   Updated: 2018/07/25 01:32:08 by alanter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	flag_int(t_printf *data)
{
	...
	if (FLAG[6] > 0 && FLAG[6] > ((int)ft_strlen(TO_ADD) - NEG))
	{
	...
	if ((FLAG[0] == 1 && (TYPE == 'x' || TYPE == 'X')) || TYPE == 'p')
		TO_ADD = (ft_atoi(TO_ADD) > 0 || TYPE == 'p') || (*TO_ADD >= 'a'
				&& *TO_ADD <= 'f') ? ft_statjoin("0x", &TO_ADD, 0, 0) : TO_ADD;
	if (FLAG[1] == 1 && ft_strchr("idD", TYPE) && (ft_atoi(TO_ADD) >= 0))
		TO_ADD = ft_statjoin("+", &TO_ADD, 0, 0);
	if (FLAG[2] == 1 && ft_strchr("idD", TYPE) && !(NEG))
		TO_ADD = ft_statjoin(" ", &TO_ADD, 0, 0);
